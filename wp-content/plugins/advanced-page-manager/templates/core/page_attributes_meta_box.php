...
if( $post->post_type == 'page' ){
	if( count(get_page_templates()) ){
		...
		if( $is_page_in_apm_tree ){
			...
			//$template = !empty($post->page_template) ? $post->page_template : false; //$post->page_template is not set in wp-admin/includes/post.php::get_post_to_edit()
		...
		?>
		<span><?php _e('No Page Template found in current theme directory!') ?></span>
		<?php
	}
} 
?>
