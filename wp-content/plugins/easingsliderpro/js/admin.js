;(function($) {
    ...
    window.SlideView = wp.media.View.extend({
        ...
        initialize: function() {
            ...
            $('.delete-images').bind('click', function(event) {
                ...
                if ( confirm( easingsliderpro.delete_images ) )
            ...
            $(document).delegate('.delete-button', 'click', function(event) {
                ...
                if ( confirm( easingsliderpro.delete_image ) )
    ...
    window.EditSlideView = wp.media.View.extend({
        ...
        initialize: function() {
            ...
            /** Create deep copy of attributes (for restoration if changes are discarded) */
            ...
            /** Toggle tabs. We use jQuery instead of backbone events because of the "this" context we require */
            $(document).delegate('.media-menu-item', 'click', function(event) {
                ...
                /** Reset 'active' class */
                $('.media-menu-item', self.$el).removeClass('active');
                $(this).addClass('active');

                /** Toggle the appropriate tab */
                $('.media-tab', self.$el).each(function() {
                    if ( this.id === id )
                        $(this).show();
                    else
                        $(this).hide();
    ...
    window.AddImageView = Backbone.View.extend({
        ...
        frameProperties: {
            title: easingsliderpro.media_upload.title,
            button: easingsliderpro.media_upload.button,
            multiple: true
        },
    ...
    window.ChangeImageView = AddImageView.extend({
        ...
        frameProperties: {
            title: easingsliderpro.media_upload.title,
            button: easingsliderpro.media_upload.change,
            multiple: false
        },
    ...
    $('.trash a').bind('click', function(event) {
        if ( !confirm( easingsliderpro.delete_slideshow ) )
            return false;
    });
    $('#doaction, #doaction2').bind('click', function(event) {
        var id = event.target.id.replace('do', '');
        if ( $('#'+ id).val() == 'delete' )
            if ( !confirm( easingsliderpro.delete_slideshows ) )
                return false;
    });

    /** Select all slideshows (import/export page) functionality */
    $('.multiple-checkbox .select-all').bind('click', function(event) {
    ...
    $('.warn').bind('click', function() {
        if ( !confirm( easingsliderpro.warn ) )
            return false;
    });
    $('.warn_reset').bind('click', function() {
        if ( !confirm( easingsliderpro.warn_reset ) )
            return false;
    });
